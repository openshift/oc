...
__oc_handle_go_custom_completion()
{
    __oc_debug "${FUNCNAME[0]}: cur is ${cur}, words[*] is ${words[*]}, #words[@] is ${#words[@]}"
    ...
    __oc_debug "${FUNCNAME[0]}: lastParam ${lastParam}, lastChar ${lastChar}"

    if [ -z "${cur}" ] && [ "${lastChar}" != "=" ]; then
        # If the last parameter is complete (there is a space following it)
        # We add an extra empty parameter so we can indicate this to the go method.
        __oc_debug "${FUNCNAME[0]}: Adding extra empty parameter"
        requestComp="${requestComp} \"\""
    fi

    __oc_debug "${FUNCNAME[0]}: calling ${requestComp}"
    ...
    __oc_debug "${FUNCNAME[0]}: the completion directive is: ${directive}"
    __oc_debug "${FUNCNAME[0]}: the completions are: ${out[*]}"

    if [ $((directive & shellCompDirectiveError)) -ne 0 ]; then
        # Error code.  No completion.
        __oc_debug "${FUNCNAME[0]}: received error from custom completion go code"
        return
    else
        if [ $((directive & shellCompDirectiveNoSpace)) -ne 0 ]; then
            if [[ $(type -t compopt) = "builtin" ]]; then
                __oc_debug "${FUNCNAME[0]}: activating no space"
                compopt -o nospace
            fi
        fi
        if [ $((directive & shellCompDirectiveNoFileComp)) -ne 0 ]; then
            if [[ $(type -t compopt) = "builtin" ]]; then
                __oc_debug "${FUNCNAME[0]}: activating no file completion"
                compopt +o default
            fi
        fi
    fi
    ...
    if [ $((directive & shellCompDirectiveFilterFileExt)) -ne 0 ]; then
        ...
        if [ -n "$subdir" ]; then
            ...
            __oc_handle_subdirs_in_dir_flag "$subdir"
        else
...
_oc_adm_build-chain()
{
    ...
    flags_completion+=("__oc_handle_go_custom_completion")
    flags+=("--context=")
...
_oc_adm_inspect()
{
    ...
    local_nonpersistent_flags+=("--dest-dir=")
    flags+=("--events-file=")
    two_word_flags+=("--events-file")
    local_nonpersistent_flags+=("--events-file")
    local_nonpersistent_flags+=("--events-file=")
    flags+=("--since=")
...
_oc_adm_prune_deployments()
{
    ...
    local_nonpersistent_flags+=("--orphans")
    flags+=("--replica-sets")
    local_nonpersistent_flags+=("--replica-sets")
    flags+=("--as=")
    two_word_flags+=("--as")
    flags+=("--as-group=")
    two_word_flags+=("--as-group")
...
_oc_adm_release_extract()
{
    ...
    local_nonpersistent_flags+=("--command-os=")
    flags+=("--credentials-requests")
    local_nonpersistent_flags+=("--credentials-requests")
    flags+=("--dir=")
    two_word_flags+=("--dir")
    local_nonpersistent_flags+=("--dir")
    local_nonpersistent_flags+=("--dir=")
...
_oc_adm_release_info()
{
    ...
    flags+=("--skip-bug-check")
    local_nonpersistent_flags+=("--skip-bug-check")
    flags+=("--skip-verification")
    local_nonpersistent_flags+=("--skip-verification")
    flags+=("--template=")
    two_word_flags+=("--template")
...
_oc_adm_top()
{
    ...
    if [[ -z "${BASH_VERSION}" || "${BASH_VERSINFO[0]}" -gt 3 ]]; then
        command_aliases+=("imagestreams")
        aliashash["imagestreams"]="imagestreams"
        command_aliases+=("is")
        aliashash["is"]="imagestreams"
    fi
    commands+=("node")
    if [[ -z "${BASH_VERSION}" || "${BASH_VERSINFO[0]}" -gt 3 ]]; then
        command_aliases+=("no")
...
_oc_adm_upgrade()
{
    ...
    flags+=("--force")
    local_nonpersistent_flags+=("--force")
    flags+=("--include-not-recommended")
    local_nonpersistent_flags+=("--include-not-recommended")
    flags+=("--to=")
    two_word_flags+=("--to")
    local_nonpersistent_flags+=("--to")
    local_nonpersistent_flags+=("--to=")
...
_oc_config_delete-user()
{
    last_command="oc_config_delete-user"
...
_oc_config_get-users()
{
    last_command="oc_config_get-users"
...
_oc_create_build()
{
    ...
    local_nonpersistent_flags+=("--context-dir=")
    flags+=("--dockerfile-contents=")
    two_word_flags+=("--dockerfile-contents")
    local_nonpersistent_flags+=("--dockerfile-contents")
    local_nonpersistent_flags+=("--dockerfile-contents=")
    flags+=("--dockerfile-path=")
    two_word_flags+=("--dockerfile-path")
    local_nonpersistent_flags+=("--dockerfile-path")
    local_nonpersistent_flags+=("--dockerfile-path=")
    flags+=("--dry-run")
    local_nonpersistent_flags+=("--dry-run")
    flags+=("--env=")
    ...
    local_nonpersistent_flags+=("--from-image=")
    flags+=("--image-optimization-policy=")
    two_word_flags+=("--image-optimization-policy")
    local_nonpersistent_flags+=("--image-optimization-policy")
    local_nonpersistent_flags+=("--image-optimization-policy=")
    flags+=("--output=")
    two_word_flags+=("--output")
    two_word_flags+=("-o")
    ...
    two_word_flags+=("--source-revision")
    local_nonpersistent_flags+=("--source-revision")
    local_nonpersistent_flags+=("--source-revision=")
    flags+=("--strategy=")
...
_oc_create_ingress()
{
    last_command="oc_create_ingress"
...
_oc_debug()
{
    ...
    local_nonpersistent_flags+=("-o")
    flags+=("--preserve-pod")
    local_nonpersistent_flags+=("--preserve-pod")
    flags+=("--quiet")
    flags+=("-q")
    local_nonpersistent_flags+=("--quiet")
    local_nonpersistent_flags+=("-q")
...
_oc_new-app()
{
    ...
    flags+=("--allow-missing-template-keys")
    local_nonpersistent_flags+=("--allow-missing-template-keys")
    flags+=("--as-deployment-config")
    local_nonpersistent_flags+=("--as-deployment-config")
    flags+=("--as-test")
...
_oc_set_probe()
{
    ...
    local_nonpersistent_flags+=("--show-managed-fields")
    flags+=("--startup")
    local_nonpersistent_flags+=("--startup")

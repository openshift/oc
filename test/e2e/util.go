...
// isMicroShiftCluster returns true if the cluster is a MicroShift cluster.
// Use this for tests that need to adjust behavior based on cluster type.
func isMicroShiftCluster(oc *CLI) bool {
	// MicroShift cluster contains "microshift-version" configmap in "kube-public" namespace
	// This is a more reliable detection method than checking for the microshift binary
	output, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("configmap", "microshift-version", "-n", "kube-public", "-o", "jsonpath={.data.version}").Output()
	if err != nil {
		// Configmap not found or error accessing it - not a MicroShift cluster
		return false
	}
	if strings.TrimSpace(output) != "" {
		e2e.Logf("MicroShift cluster detected with version: %s", strings.TrimSpace(output))
		return true
	}
	return false
}
...
// skipOnTransientNetworkError checks if the error output contains transient network errors
// and skips the test if so. Use this after command execution that depends on external registries.
func skipOnTransientNetworkError(output string, err error) {
	if err != nil && isTransientNetworkError(output) {
		skipMsg := fmt.Sprintf("Skipping test due to transient network error: %s", output)
		e2e.Warningf("SKIPPING TEST: %s", skipMsg)
		g.Skip(skipMsg)
	}
}

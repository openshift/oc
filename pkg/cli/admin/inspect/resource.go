...
func InspectResource(ctx context.Context, info *resource.Info, rCtx *resourceContext, o *InspectOptions) error {
	if rCtx.visited.Has(infoToContextKey(info)) {
		klog.V(1).Infof("Skipping previously-inspected resource: %q ...", infoToContextKey(info))
		return nil
	}
	rCtx.visited.Insert(infoToContextKey(info))
	...
	case configv1.GroupVersion.WithResource("clusteroperators").GroupResource():
		...
		errs := []error{}
		if err := o.gatherConfigResourceData(ctx, path.Join(o.DestDir, "/cluster-scoped-resources/config.openshift.io"), rCtx); err != nil {
			errs = append(errs, err)
		}

		// then, gather operator.openshift.io resource data
		if err := o.gatherOperatorResourceData(ctx, path.Join(o.DestDir, "/cluster-scoped-resources/operator.openshift.io"), rCtx); err != nil {
			errs = append(errs, err)
		}
		...
		if err := gatherRelatedObjects(ctx, rCtx, unstr, o); err != nil {
			errs = append(errs, err)
		}
	...
	case corev1.SchemeGroupVersion.WithResource("namespaces").GroupResource():
		...
		for _, resource := range resourcesToCollect {
			if rCtx.visited.Has(resourceToContextKey(resource, info.Name)) {
				continue
			}
			resourceInfos, err := groupResourceToInfos(o.configFlags, resource, info.Name, rCtx.serverResources)
			if err != nil {
				errs = append(errs, err)
				continue
			}
			for _, resourceInfo := range resourceInfos {
				if err := InspectResource(ctx, resourceInfo, rCtx, o); err != nil {
					errs = append(errs, err)
					continue
				}
			}
...
func gatherGenericObject(ctx context.Context, rCtx *resourceContext, info *resource.Info, o *InspectOptions) error {
	errs := []error{}
	unstr, ok := info.Object.(*unstructured.Unstructured)
	if ok {
		// obtain associated objects for the current resource
...
func gatherRelatedObjects(ctx context.Context, rCtx *resourceContext, unstr *unstructured.Unstructured, o *InspectOptions) error {
	relatedObjReferences, err := obtainRelatedObjects(unstr)
	if err != nil {
		return err
	}

	return gatherMoreObjects(ctx, rCtx, o, relatedObjReferences...)
}

func gatherMoreObjects(ctx context.Context, rCtx *resourceContext, o *InspectOptions, relatedObjReferences ...*configv1.ObjectReference) error {
	errs := []error{}
	for _, relatedRef := range relatedObjReferences {
		if rCtx.visited.Has(objectRefToContextKey(relatedRef)) {
			continue
		}
		...
		for _, relatedInfo := range relatedInfos {
			if err := InspectResource(ctx, relatedInfo, rCtx, o); err != nil {
				errs = append(errs, fmt.Errorf("skipping gathering %s due to error: %v", objectReferenceToString(relatedRef), err))
				continue
			}
		}
...
func gatherNamespaces(ctx context.Context, rCtx *resourceContext, o *InspectOptions, namespaces ...string) error {

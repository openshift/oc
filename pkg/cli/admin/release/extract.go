...
func NewExtract(f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	flags.StringVar(&o.CommandOperatingSystem, "command-os", o.CommandOperatingSystem, "Override which operating system command is extracted (mac, windows, linux). You may specify '*' to extract all tool archives.")
	flags.StringVar(&o.CommandArch, "command-arch", o.CommandArch, "Override the arch of the command which will be extracted (amd64, arm64, ...). If --command-os is '*', this is ignored")

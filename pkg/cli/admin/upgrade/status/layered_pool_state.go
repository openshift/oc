...
func (l *LayeredPoolState) IsLayered() bool {
	...
	if _, ok := l.pool.Labels["machineconfiguration.openshift.io/layering-enabled"]; ok {
		return true
	}
	return false
...
func (l *LayeredPoolState) GetOSImage() string {
	osImage := l.pool.Annotations["machineconfiguration.openshift.io/newestImageEquivalentConfig"]
	return osImage
}
...
func (l *LayeredPoolState) HasOSImage() bool {
	...
	val, ok := l.pool.Annotations["machineconfiguration.openshift.io/newestImageEquivalentConfig"]
	return ok && val != ""
}

...
func TestAssessControlPlaneStatus_Operators(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expected: operators{Total: 2, Updating: 1, Waiting: 1, UpdatingClusterOperators: []UpdatingClusterOperator{{Name: "two", Condition: &configv1.ClusterOperatorStatusCondition{
				Type:               "Progressing",
				Status:             "True",
		...
		},
		{
			...
			expected: operators{Total: 2, Unavailable: 1, Updating: 1, Waiting: 1, UpdatingClusterOperators: []UpdatingClusterOperator{{Name: "two", Condition: &configv1.ClusterOperatorStatusCondition{
				Type:               "Progressing",
				Status:             "True",
				LastTransitionTime: metav1.Date(2023, 12, 1, 23, 23, 0, 0, time.UTC),
				Reason:             "ProgressingTowardsDesired",

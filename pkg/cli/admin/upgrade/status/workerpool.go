...
func assessNodesStatus(cv *configv1.ClusterVersion, pool mcfgv1.MachineConfigPool, nodes []corev1.Node, machineConfigs []mcfgv1.MachineConfig, multiArchMigration bool) ([]nodeDisplayData, []updateInsight) {
	...
	for _, node := range nodes {
		...
		isUpdated := foundCurrent && isLatestUpdateHistoryVersionEqualTo(cv.Status.History, currentVersion) &&
			...
			(!multiArchMigration || !ok || (annotationMatched && desiredAnnotationUpdated))

...
func (l *LayeredNodeState) isDesiredMachineConfigEqualToPool(mcp *mcfgv1.MachineConfigPool) bool {
	return l.node.Annotations[("machineconfiguration.openshift.io/desiredConfig")] == mcp.Spec.Configuration.Name
}
...
func (l *LayeredNodeState) isDesiredImageEqualToPool(mcp *mcfgv1.MachineConfigPool) bool {
	return l.isImageAnnotationEqualToPool("machineconfiguration.openshift.io/desiredImage", mcp)
}
...
func (l *LayeredNodeState) isCurrentImageEqualToPool(mcp *mcfgv1.MachineConfigPool) bool {
	return l.isImageAnnotationEqualToPool("machineconfiguration.openshift.io/currentImage", mcp)
}
...
func (l *LayeredNodeState) SetDesiredStateFromPool(mcp *mcfgv1.MachineConfigPool) {
	...
	node.Annotations[("machineconfiguration.openshift.io/desiredConfig")] = mcp.Spec.Configuration.Name

	lps := NewLayeredPoolState(mcp)

	if lps.IsLayered() && lps.HasOSImage() {
		node.Annotations[("machineconfiguration.openshift.io/desiredImage")] = lps.GetOSImage()
	} else {
		delete(node.Annotations, "machineconfiguration.openshift.io/desiredImage")
	}
...
func isNodeConfigDone(node *corev1.Node) bool {
	cconfig, ok := node.Annotations[("machineconfiguration.openshift.io/currentConfig")]
	if !ok || cconfig == "" {
		return false
	}

	dconfig, ok := node.Annotations[("machineconfiguration.openshift.io/desiredConfig")]
	if !ok || dconfig == "" {
		return false
	}
...
func isNodeImageDone(node *corev1.Node) bool {
	desired, desiredOK := node.Annotations[("machineconfiguration.openshift.io/desiredImage")]
	current, currentOK := node.Annotations[("machineconfiguration.openshift.io/currentImage")]
...
func isNodeDoneAt(node *corev1.Node, pool *mcfgv1.MachineConfigPool) bool {
	return isNodeDone(node) && node.Annotations[("machineconfiguration.openshift.io/currentConfig")] == pool.Spec.Configuration.Name
}
...
func isNodeMCDState(node *corev1.Node, state string) bool {
	dstate, ok := node.Annotations[("machineconfiguration.openshift.io/state")]
	if !ok || dstate == "" {
		return false
	}
...
func isNodeMCDFailing(node *corev1.Node) bool {
	...
	return isNodeMCDState(node, "Degraded") ||
		isNodeMCDState(node, "Unreconcilable")
}

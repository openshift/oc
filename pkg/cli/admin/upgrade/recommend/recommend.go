...
func New(f kcmdutil.Factory, streams genericiooptions.IOStreams) *cobra.Command {
	...
	if kcmdutil.FeatureGate("OC_ENABLE_CMD_UPGRADE_RECOMMEND_ACCEPTED_RISKS").IsEnabled() {
		flags.StringSliceVar(&o.acceptedRisks, "accepted-risk", o.acceptedRisks, "Comma-delimited slugs for risks that you find acceptable.  With --version, any unaccepted risks will result in a non-zero exit code.")
	}
	...
	if o.precheckEnabled {
		...
		for _, condition := range conditions {
			if condition.Status == metav1.ConditionTrue {
				...
				alertname := condition.Type // FIXME: actually pull out the alert name, if folks seem positive on the overall commit
				risks.Insert(alertname)
				if acceptedRisks.Has(alertname) {
		...
		if len(unhappyConditions) > 0 {
			...
			for _, c := range conditions {
				alertname := c.Type // FIXME: actually pull out the alert name, if folks seem positive on the overall commit
				context := ""
				if acceptedRisks.Has(alertname) {
					context = "accepted "
				}

...
func New(f kcmdutil.Factory, streams genericiooptions.IOStreams) *cobra.Command {
	...
	flags.StringVar(&o.version, "version", o.version, "Select a particular target release to display by version.")
...
func (o *options) Complete(f kcmdutil.Factory, cmd *cobra.Command, args []string) error {
	...
	if o.version != "" {
		...
		if _, err := semver.Parse(o.version); err != nil {
			return fmt.Errorf("cannot parse SemVer target %q: %v", o.version, err)
		}
	}
...
func (o *options) Run(ctx context.Context) error {
	...
	if o.version != "" {
		target, err := semver.Parse(o.version)
		if err != nil {
			return fmt.Errorf("cannot parse SemVer target %q: %v", o.version, err)
		}

		if len(majorMinorBuckets) == 0 {
			return fmt.Errorf("no updates available, so cannot display context for the requested release %s", target)
		}

		if major, ok := majorMinorBuckets[target.Major]; !ok {
			return fmt.Errorf("no updates to %d available, so cannot display context for the requested release %s", target.Major, target)
		} else if minor, ok := major[target.Minor]; !ok {
			return fmt.Errorf("no updates to %d.%d available, so cannot display context for the requested release %s", target.Major, target.Minor, target)
		} else {
			for _, update := range minor {
				if update.Release.Version == o.version {
					fmt.Fprintln(o.Out)
	...
	for _, major := range majors {
		...
		for _, minor := range minors {
			...
			fmt.Fprintf(w, "  VERSION\tIMAGE\tISSUES\n")
			...
			for i, update := range majorMinorBuckets[major][minor] {
				...
				if c == nil {
					...
					fmt.Fprintf(w, "  %s\t%s\t%s\n", update.Release.Version, update.Release.Image, c.Reason)
				} else {

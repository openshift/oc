...
// satisfies *Context AlternativeRepositoryStrategy interface
...
func NewSimpleLookupICSPStrategy(file string, client operatorv1alpha1client.ImageContentSourcePolicyInterface) registryclient.AlternateRepositoryStrategy {
...
// WithAlternateSources returns a list of possible image references for the locator image reference, gathered from image content source policies, or an error
func (s *simpleLookupICSP) WithAlternateSources(ctx context.Context, locator reference.DockerImageReference, reason error) (alternateRepositories []reference.DockerImageReference, err error) {
	s.lock.Lock()
	defer s.lock.Unlock()

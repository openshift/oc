...
import (
	...
	regauth "github.com/openshift/library-go/pkg/image/registryclient/v2/auth"
	"github.com/openshift/library-go/pkg/image/registryclient/v2/clienterrors"
	registryclient "github.com/openshift/library-go/pkg/image/registryclient/v2/internal/client"
	"github.com/openshift/library-go/pkg/image/registryclient/v2/internal/client/auth"
	"github.com/openshift/library-go/pkg/image/registryclient/v2/internal/client/auth/challenge"
...
func (c *Context) cachedTransport(rt http.RoundTripper, host string, scopes []regauth.Scope, ref imagereference.DockerImageReference) http.RoundTripper {
	...
	sort.Strings(names)
	authScopes := make([]regauth.Scope, 0, len(scopeNames))
	for _, s := range names {
...
func (c *Context) scopes(repoName string) []regauth.Scope {
	...
	if len(c.Actions) == 0 {
		scopes = append(scopes, regauth.RepositoryScope{Repository: repoName, Actions: []string{"pull"}})
	} else {
		scopes = append(scopes, regauth.RepositoryScope{Repository: repoName, Actions: c.Actions})
	}
	return scopes
}
